...
import {priceSamples,ranges} from '../dist/history.js';import {newestHistory,availableHistory} from '../dist/history-cache.js';
test('every saved asset/range stays drawable after an API failure',async()=>{
 ...
 const context={asset:null,current:()=>context.asset,data:{},chartDays:90,historyLoadTimer:null,bundledHistory:new Map(),historyBundlesLoaded:new Set(),ensureHistoryBundle:async()=>{},historyCache:new Map(),historyPending:new Set(),historyAttempted:new Set(),historyErrors:new Map(),selected:null,priceSamples,newestHistory,availableHistory,ranges,Date,setTimeout:()=>0,clearTimeout:()=>{},when:t=>new Date(t).toISOString(),node:()=>({}),drawPriceChart:(svg,readout,rows)=>{drawn=rows},saveHistoryCache:()=>{},chartStorage:null,loadHistory:async()=>{throw Error('Provider request failed (429)')},$:id=>{if(!elements.has(id))elements.set(id,{children:[],append(){},textContent:''});return elements.get(id)}};
 ...
 for(const file of fs.readdirSync(dir).filter(f=>f.endsWith('.json'))){
  ...
  for(const [days,h]of Object.entries(saved)){context.chartDays=days==='max'?days:Number(days);context.bundledHistory.set(`${id}:${days}`,h);await vm.runInContext(`refreshHistory(${JSON.stringify(id)},${JSON.stringify(days==='max'?days:Number(days))})`,context);assert.ok(drawn.length>=2,`${id}:${days} lost fallback`);assert.match(elements.get('chartInfo').textContent,/429/);checked++;}
 }
