...
import {newestHistory,availableHistory} from '../dist/history-cache.js';
test('actual chart rendering keeps all Monero ranges visible after live refresh fails',async()=>{
 ...
 const context={current:()=>({id:'monero',name:'Monero'}),data:{},chartDays:90,historyLoadTimer:null,bundledHistory:new Map(),historyBundlesLoaded:new Set(['monero']),ensureHistoryBundle:async()=>{},historyCache:new Map(Object.entries(saved).map(([days,h])=>[`monero:${days}`,h])),historyPending:new Set(),historyAttempted:new Set(),historyErrors:new Map(),selected:'monero',priceSamples,newestHistory,availableHistory,ranges,Date,setTimeout:()=>0,clearTimeout:()=>{},when:t=>new Date(t).toISOString(),node:()=>({}),drawPriceChart:(svg,readout,rows)=>{lastRows=rows},saveHistoryCache:()=>{},chartStorage:null,loadHistory:async()=>{throw Error('Provider request failed (429)')},$:id=>{if(!elements.has(id))elements.set(id,{children:[],append(){},textContent:''});return elements.get(id)}};
 vm.createContext(context);vm.runInContext(render+'\n'+refresh,context);
 for(const {days}of ranges){context.chartDays=days;await vm.runInContext(`refreshHistory('monero',${JSON.stringify(days)})`,context);assert.ok(lastRows.length>2,`${days} day chart disappears`);assert.match(elements.get('chartInfo').textContent,/429/);assert.ok(elements.get('chartTitle').textContent.includes(ranges.find(r=>r.days===days).label));}
});
