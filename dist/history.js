...
export const ranges=[{days:1,label:'1D',name:'1 day'},{days:7,label:'7D',name:'7 days'},{days:30,label:'1M',name:'1 month'},{days:90,label:'3M',name:'3 months'},{days:180,label:'6M',name:'6 months'}];
...
export function priceSamples(raw,now=Date.now(),days=180){
 const rows=(Array.isArray(raw)?raw:[]).filter(p=>Array.isArray(p)&&Number.isFinite(p[0])&&Number.isFinite(p[1])&&p[1]>0&&p[0]<=now&&p[0]>=now-days*DAY).sort((a,b)=>a[0]-b[0]);
...
export async function loadHistory(id,{days=90,fetcher=fetch,now=Date.now()}={}){
 ...
 try{
  const data=await request(`https://api.coingecko.com/api/v3/coins/${encodeURIComponent(id)}/market_chart?vs_currency=usd&days=${days}`,fetcher);
  const prices=priceSamples(data.prices,now,days);
  if(prices.length<2)throw Error('Insufficient chart data');
  return {prices,asOf:now,source:'CoinGecko',currency:'USD',description:'Historical price samples',url:`https://www.coingecko.com/en/coins/${encodeURIComponent(id)}`};
 }catch(error){
  ...
  try{
   ...
   const limit=days===1?288:days===7?168:days===30?180:days;
   ...
   return {prices,asOf:now,source:'MEXC',currency:'USDT',description:`Completed ${interval} closes · single exchange`,url:'https://www.mexc.com/exchange/XTM_USDT'};
